...
export default function Page() {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-transparent text-foreground">
      <div className="fixed inset-0 z-0 bg-[url('https://hebbkx1anhila5yf.public.blob.vercel-storage.com/9aa5dca0-2bc0-47d6-bad0-9fc9c181d6d5-lPtm1gzASls3ZGlxkNlmkoZ91TOHLV.jpeg')] bg-cover bg-center" aria-hidden="true" />
      ...
      <header className="absolute inset-x-0 top-0 z-50">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-5 lg:px-8">
          <a href="#start" className="flex items-center gap-3" aria-label="Dla Cla Cli - strona główna">
            <span className="flex size-10 items-center justify-center rounded-xl bg-primary text-foreground"><Sparkles size={20} /></span>
          ...
          <nav className="hidden items-center gap-8 text-sm font-medium text-white/80 md:flex">
            ...
            <a href="#kontakt" className="rounded-full bg-primary px-5 py-3 text-foreground transition-transform hover:scale-105">Bezpłatna wycena <ArrowRight className="ml-2 inline" size={16} /></a>
          </nav>
      ...
      <section id="uslugi" className="relative z-10 bg-transparent py-20 lg:py-28"><div className="mx-auto max-w-7xl px-5 lg:px-8"><div className="mb-12 flex flex-col justify-between gap-5 md:flex-row md:items-end"><div><p className="mb-3 text-sm font-bold uppercase tracking-[0.16em] text-accent-foreground">Co możemy dla Ciebie zrobić</p><h2 className="max-w-xl text-balance font-mono text-3xl font-bold tracking-tight text-foreground sm:text-5xl">Czystość dopasowana do Twojego życia.</h2></div><p className="max-w-sm leading-6 text-muted-foreground">Od szybkiego odświeżenia po kompleksowe porządki. Ty wybierasz zakres, my dbamy o każdy szczegół.</p></div><div className="grid gap-4 md:grid-cols-3">{services.map(({ icon: Icon, title, text }) => <article key={title} className="group rounded-2xl border border-border bg-card/55 p-7 transition-all hover:-translate-y-1 hover:border-accent hover:shadow-lg"><div className="mb-12 flex size-12 items-center justify-center rounded-xl bg-accent/15 text-accent-foreground"><Icon size={23} /></div><h3 className="font-mono text-xl font-bold text-foreground">{title}</h3><p className="mt-3 leading-6 text-muted-foreground">{text}</p><a href="#kontakt" className="mt-6 inline-flex items-center text-sm font-bold text-foreground">Dowiedz się więcej <ArrowRight className="ml-2 transition-transform group-hover:translate-x-1" size={16} /></a></article>)}</div></div></section>

      <section id="dlaczego-my" className="relative z-10 bg-transparent py-20 lg:py-28"><div className="mx-auto max-w-7xl px-5 lg:px-8"><div className="grid gap-12 lg:grid-cols-[0.8fr_1.2fr] lg:items-center"><div><p className="mb-3 text-sm font-bold uppercase tracking-[0.16em] text-accent-foreground">Prosto i bez stresu</p><h2 className="text-balance font-mono text-3xl font-bold tracking-tight text-foreground sm:text-5xl">Porządek zaczyna się od dobrej rozmowy.</h2><p className="mt-5 max-w-md leading-7 text-muted-foreground">Działamy lokalnie, znamy Toruń i wiemy, że każda przestrzeń ma swoje potrzeby. Dlatego nie proponujemy gotowych pakietów — słuchamy i dopasowujemy usługę.</p></div><div className="grid gap-7">{steps.map(([num, title, text]) => <div key={num} className="flex gap-5 border-b border-border pb-7 last:border-0"><span className="font-mono text-sm font-bold text-accent-foreground">{num}</span><div><h3 className="font-mono text-lg font-bold text-foreground">{title}</h3><p className="mt-2 leading-6 text-muted-foreground">{text}</p></div></div>)}</div></div></div></section>

      <section id="opinie" className="relative z-10 bg-transparent py-20 text-foreground lg:py-24"><div className="mx-auto max-w-7xl px-5 lg:px-8"><div className="flex flex-col items-start justify-between gap-8 md:flex-row md:items-end"><div><div className="mb-5 flex gap-1 text-accent">{[1,2,3,4,5].map((item) => <Star key={item} size={18} fill="currentColor" />)}</div><blockquote className="max-w-3xl text-balance font-mono text-2xl font-bold leading-snug sm:text-4xl">„W końcu wracam do domu i naprawdę odpoczywam. Dla Cla Cli to nie tylko sprzątanie — to odzyskany czas.”</blockquote></div><p className="shrink-0 text-sm text-white/60">— Marta, Toruń / Rubinkowo</p></div></div></section>

      <section className="relative z-10 bg-transparent py-20 lg:py-28"><div className="mx-auto max-w-3xl px-5 lg:px-8"><p className="mb-3 text-sm font-bold uppercase tracking-[0.16em] text-accent-foreground">Masz pytania?</p><h2 className="font-mono text-3xl font-bold tracking-tight text-foreground sm:text-5xl">Warto wiedzieć</h2><div className="mt-8 divide-y divide-border border-y border-border">{faqs.map(([question, answer]) => <details key={question} className="group py-5"><summary className="flex cursor-pointer list-none items-center justify-between gap-4 font-semibold text-foreground [&::-webkit-details-marker]:hidden"><span>{question}</span><ChevronDown className="shrink-0 transition-transform group-open:rotate-180" size={20} /></summary><p className="max-w-2xl pt-3 leading-6 text-muted-foreground">{answer}</p></details>)}</div></div></section>

      <section id="kontakt" className="relative z-10 bg-accent/85 px-5 py-16 text-accent-foreground lg:px-8 lg:py-20"><div className="mx-auto flex max-w-7xl flex-col justify-between gap-8 md:flex-row md:items-end"><div><p className="mb-3 text-sm font-bold uppercase tracking-[0.16em]">Zacznijmy od czystej kartki</p><h2 className="max-w-2xl text-balance font-mono text-3xl font-bold tracking-tight sm:text-5xl">Gotowy na więcej wolnego czasu?</h2><p className="mt-4 max-w-lg leading-6 opacity-80">Napisz do nas lub zadzwoń. Przygotujemy bezpłatną wycenę dopasowaną do Twojej przestrzeni.</p></div><div className="flex flex-col gap-3 sm:flex-row"><a href="tel:+48500123456" className="inline-flex min-h-12 items-center justify-center rounded-full bg-primary px-6 font-bold text-foreground"><Phone className="mr-2" size={17} /> 500 123 456</a><a href="mailto:kontakt@dlaclacli.pl" className="inline-flex min-h-12 items-center justify-center rounded-full border border-accent-foreground/25 px-6 font-bold">Napisz wiadomość <ArrowRight className="ml-2" size={17} /></a></div></div></section>
